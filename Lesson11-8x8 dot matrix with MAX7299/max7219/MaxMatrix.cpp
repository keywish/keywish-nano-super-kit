...
/*void MaxMatrix::putAnimationMouth(unsigned long int aniMouth, int index) {

  ledmatrix.writeFull(getAnimShape(aniMouth, index));
  }


  void MaxMatrix::putMouth(unsigned long int mouth, bool predefined) {
  if (predefined) {
    MaxMatrix::writeFull(getMouthShape(mouth));
  }
  else {
    MaxMatrix::writeFull(mouth);
  }
  }
*/
